...
export const WorkspaceInvitesSection = ({
  ...
  const utils = api.useUtils();
  const { data: invite, isLoading: isGetInviteForWorkspaceLoading } =
    api.invite.getInviteForWorkspace.useQuery({
      workspaceId,
    });
  const { mutate: createInvite, isLoading: isCreateInviteLoading } =
  ...
  const { mutate: deleteInvite, isLoading: isDeleteInviteLoading } =
  ...
  return (
    <>
      ...
      <Table>
        ...
        <Table.Tbody>
          {isGetInviteForWorkspaceLoading ? (
            <Table.Tr>
              <Table.Td colSpan={3}>
            ...
            <Table.Tr key={invite.id}>
              ...
              <Table.Td className={"text-end"}>
                <CopyButton
                  value={`${window.location.origin}/invite/${invite.id}`}
                >
                  {({ copied, copy }) => (
                    <ActionIcon
                      onClick={copy}
                  ...
                  <ActionIcon
                    onClick={() => {
                      modals.openConfirmModal({
                        ...
                        onConfirm: () => deleteInvite({ inviteId: invite.id }),
                      });
                    }}
                    variant={"light"}
                    className={"ml-3"}
          ...
          <Button
            ...
            onClick={() => {
              modals.openConfirmModal({
                ...
                onConfirm: () => createInvite({ workspaceId }),
              });
            }}
            className={"mt-2"}
          >
          ...
          <Button
            ...
            onClick={() =>
              modals.openConfirmModal({
                ...
                onConfirm: () => regenerateInvite({ inviteId: invite.id }),
              })
            }
          >
            Refresh invite
